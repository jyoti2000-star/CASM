...
class CodeGenerator:
    ...
    def generate(self, ast: ProgramNode) -> str:
        ...
        if self.data_section:
            result.append(self.platform.data_directive)
            result.extend(self.data_section)
            result.append("")
        
        # Add bss section
        if self.bss_section:
            result.append(f"{self.platform.section_prefix} .bss")
            result.extend(self.bss_section)
            result.append("")
        
        # Add text section and entry point
        result.append(self.platform.text_directive)
